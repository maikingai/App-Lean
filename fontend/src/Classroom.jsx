...
export default function Classroom() {
    ...
    ;(async () => {
      try {
        ...
        if (m) {
          ...
          setCurrentUser({ display_name: display, role: localStorage.getItem('role') || m.role || '' })
        }
      }
    })()
  ...
  // ---------------- Active class info ----------------
  const [classInfo, setClassInfo] = useState({ id: null, name: 'Class', code: '' })

  useEffect(() => {
    try {
      const active = localStorage.getItem('activeClassId')
      if (active) {
        // fetch class data from backend to get the real class code
        ;(async () => {
          try {
            ...
            if (res.ok) {
              ...
              const key = `classroom_${active}`
              const raw = localStorage.getItem(key)
              if (raw) {
                const parsed = JSON.parse(raw)
                setClassInfo({ id: parsed.id || active, name: parsed.name || 'Class', code: '' })
                if (Array.isArray(parsed.posts) && parsed.posts.length) setPosts(parsed.posts)
              } else {
                setClassInfo({ id: active, name: 'Class', code: '' })
              }
            }
          } catch (err) {
  ...
  const copyCode = async () => {
    const codeText = classInfo?.code || 'N/A'
    try {
  ...
  return (
    <div className="th-root">
      {/* ===== Sidebar (match Home - switch by role) ===== */}
      <StudentSidebar />

      {/* ===== Main ===== */}
      <main className="th-main th-main-rel">
        {/* Topbar */}
        <div className="th-topbar" style={{ justifyContent: 'flex-end', display: 'flex', gap: 10, alignItems: 'center' }}>
          <UserProfile />
        </div>
        ...
        <section className="th-card" style={{
          ...
          <div style={{ fontSize: 22, fontWeight: 800 }}>{classInfo.name}</div>
        </section>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '260px 1fr', gap: 18, alignItems: 'start', marginTop: 14 }}>
          ...
          <aside style={{ display: 'grid', gap: 14 }}>
            ...
            <div className="th-card" style={{ padding: 14, borderRadius: 16, boxShadow:'0 14px 36px rgba(15,23,42,.12)' }}>
              ...
              <div style={{ fontSize: 20, fontWeight: 800, color: '#111' }}>{classInfo.code || 'N/A'}</div>
            </div>
          </aside>
